...
interface SmtpPageContentInnerProps {
  smtpConfig: ISmtpConfigResponse | null;
}

function SmtpPageContentInner({ smtpConfig }: SmtpPageContentInnerProps) {
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      ...
      notifications.show({
        message: error instanceof Error ? error.message : 'Error sending test email',
        color: 'red',
      });
    } finally {
...
export default function SmtpPageContent({
  ...
  const smtpResult = use(smtpConfigPromise);

  if (!meResult.success || !meResult.data) {
    redirect('/login');
  }

  return <SmtpPageContentInner smtpConfig={smtpResult.data ?? null} />;
}
