...
export default async function LandingTourCategory({ category, queryParams }: LandingTourCategoryProps) {
  // Get all tours in this category
  const tourCategoryToursResponse = await getTourCategoryToursByTourCategoryIdAction(category.id);

  // Extract tours and filter only public ones
  const tours: ITourResponse[] = tourCategoryToursResponse.success && tourCategoryToursResponse.data
    ? tourCategoryToursResponse.data
      .map(tct => tct.tour)
      .filter((tour): tour is ITourResponse => tour !== undefined && tour.visibility === 'public')
    : [];
